...
const createCrawl = async (crawlId, crawlName, status) => {
  try {
    const response = await esClient.index({
      body: {
        crawlId,
    ...
    writeLog(`Error creating crawl: ${error}`);
    throw error;
  }
}
const updateCrawl = async (crawlId, status) => {
  try {
    ...
    writeLog(`Error updating crawl: ${error}`);
    throw error;
  }
}
// Update a document
...
const getStats = async() => {
  let uniqueCrawlNames = await getUniqueCrawlNames();
  let totalDocuments = await getTotalDocuments();
  return { uniqueCrawlNames, totalDocuments };
}
// Check if URL exists
...
export { getTotalDocuments, getStats, getUniqueCrawlNames, push, remove, search, update, urlExists, createCrawl };