...
const startCrawl = async ({ crawlId, crawlName, getImages=false,links, maxRequests, requestHandlerTimeoutSecs=30, maxRequestRetries=2, crawlSitemap=false, reIndexDuplicates=false, crawlerMaxConcurrency=5 }) => {
  var totalURLs = 0;
  var start = new Date()

  crawlLog.info(`starting crawl for ${crawlName} with id ${crawlId} at ${start.toLocaleString()}`, crawlId);
  ...
  if (crawlSitemap) {
    ...
    crawlLog.info(`Sitemap loaded with ${urls.length} URLs`, crawlId);
      ...
      crawlLog.error(`Error loading sitemap: ${error.message}`, crawlId, "error");
    }
  }
  ...
  const options = {
    ...
    requestHandler: async ({ enqueueLinks, page, request }) => {
      try {
        ...
        crawlLog.error(`Error processing ${request.url}: ${error.message}`, crawlId, "error");
      }
    },
  ...
  crawlLog.info(`Crawl ${crawlId} finished in ${end}ms. Indexed ${totalURLs} pages.`, crawlId);
...
const enqueueCrawl = async (opts) => {
  ...
  crawlLog.info(`Enqueuing crawl for ${opts.crawlName} with id ${opts.crawlId}`, opts.crawlId);
  startCrawl(opts);
  return crawlId;
};
