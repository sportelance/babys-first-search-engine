...
import { createCrawl, push, urlExists, updateCrawl } from "./database.js"
...
const startCrawl = async ({
  ...
  crawlLog.info(
    `starting crawl with ${links.length} links for ${crawlName} at ${start.toLocaleString()}`,
    crawlId
  )
  ...
  const options = {
    failedRequestHandler: ({ request }) => {
      crawlLog.warn(`Request ${request.url} failed.`)
    },
